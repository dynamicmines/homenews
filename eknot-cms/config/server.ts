      ...
      parseActiveSources: {
        task: async ({ strapi }: any) => {
          ...
          const processingPrompt = env(
            'AUTO_PARSER_PROMPT',
            'Сделай короткие городские новости для e-Knot. Пиши официально, понятно для жителей, без неподтвержденных фактов. Сохраняй все важные факты: даты, адреса, маршруты, ограничения, причины и рекомендации. Не используй фразы “смотрите в изображении” или “подробности на картинке”.'
          );
